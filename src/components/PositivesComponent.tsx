  ...
  .create({
    ...
    methods: {
      async fetchData() {
        ...
        await this.$store.direct.dispatch.Covid19ApiStore.getPositives(
    ...
    render() {
      let items: Element[] = [];
      this.positives.forEach(item => {
      let p =
          <v-list-item key={item.id}>
            <v-list-item-title>{item.prefecture}</v-list-item-title>
            {/* <v-list-item-icon>
      ...
      return (
        <div>
          ...
          <v-list class="transparent">{items}</v-list>
        </div>
      );
    },
    computed: {
      positives() {
        return (
          this.$store.direct.state.Covid19ApiStore.positivesLocalAchievements.find(
            e => e.name == this.selectedPrefecture
          )?.positives ?? []
        );
      },
      PREFECTURES() {
